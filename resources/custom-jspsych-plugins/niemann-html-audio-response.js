 ...
 * Extends the originial plugin with the option to show a start and a stop button for the recording.
 *
 **/
...
var jsPsychHtmlAudioResponse = (function (jspsych) {
  ...
  const info = {
      ...
      parameters: {
          ...
          /** EXTENSION: Whether or not to start the recording automatically. If false, the participant can start the recording by clicking on a start button. */
          start_recording_automatically: {
            type: jspsych.ParameterType.BOOL,
            default: true
          },
  ...
  class HtmlAudioResponsePlugin {
      ...
      trial(display_element, trial) {
          ...
          // EXTENSION: start recording automatically if start_recording_automatically is true
          if (trial.start_recording_automatically) {
            this.startRecording();
          }
          else {
            this.showRecordingControls(display_element, trial);
          }
          //
      }
      showDisplay(display_element, trial) {
          ...
          if (!trial.start_recording_automatically) {
              // add start button
              html += `<button class="jspsych-btn" id="start-trial">${trial.start_button_label}</button>`;
              // add stop button
              html += `<button class="jspsych-btn" id="stop-trial">${trial.stop_button_label}</button>`;
          }
          //
      ...
      setupRecordingEvents(display_element, trial) {
          ...
          this.start_event_handler = (e) => {
              ...
              if (trial.start_recording_automatically) {
                  ...
                  this.setHidingTimer(display_element, trial);
      ...
      // EXTENSION: show recording controls (start and stop button)
      showRecordingControls(display_element, trial) {
          this.showDisplay(display_element, trial);
          ...
          display_element.querySelector("#start-trial").addEventListener("click", () => {
              ...
              display_element.querySelector("#stop-trial").disabled = false;
              this.startRecording();
          });

          display_element.querySelector("#stop-trial").addEventListener("click", () => {
            display_element.querySelector("#stop-trial").disabled = true;
      ...
      // EXTENSION: set timer for hiding the stimulus
      setHidingTimer(display_element, trial) {
