...
jsPsych.plugins["prime-images"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      colors: {
        ...
        array: true,
        default: undefined,
        description: "Array of color names, same size as images array"
      },
      images_padding: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // If the colors weren't provided, we make it all black
    if (trial.colors.length == 0)
      trial.colors = Array(trial.images.length).fill("black");
    ...
    if (trial.images_padding.length == 0)
      ...
      trial.images_padding = Array(trial.images.length * 2).fill(10);
    else {
